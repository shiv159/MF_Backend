...
import com.mutualfunds.api.mutual_fund.features.portfolio.analytics.dto.YearProjection;
...
public class WealthProjectionService {
    ...
    private PortfolioStats extractFundStats(Fund fund) {
        ...
        try {
            ...
            if (meta != null) {
                Double stdDevMetric = metadataExtractor.extractMetric(meta, "stdev", "standardDeviation", "std_dev");
                double stdDev = stdDevMetric != null ? stdDevMetric / 100.0 : 0.15;

                double alpha = 0.0;
                double beta = 1.0;

                Double alphaMetric = metadataExtractor.extractMetric(meta, "alpha");
                if (alphaMetric != null) {
                    alpha = alphaMetric / 100.0;
                }
                Double betaMetric = metadataExtractor.extractMetric(meta, "beta");
                if (betaMetric != null) {
                    beta = betaMetric;
                }
